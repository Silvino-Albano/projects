  ...
  <div class="page">
    ...
    <div class="stats-big" v-if="statistik">
      <div class="sb-card primary">
        ...
        <div class="sb-label">Total Komisi Keseluruhan</div>
      </div>
    ...
    <div class="filter-bar">
      <div class="filter-wrap">
        <button
          v-for="f in ['all','unpaid','paid']" :key="f"
          class="filter-btn"
    ...
    <div v-if="loading" class="state-wrap">
      <div class="spinner"></div><p>Memuat data komisi...</p>
    </div>

    <!-- DAFTAR KOMISI -->
    <div v-else class="komisi-wrap">

      <div v-if="!filteredKomisi.length" class="state-wrap">
        <div class="empty-icon">📭</div>
        <p class="empty-text">Tidak ada data komisi.</p>
      </div>

      <div v-else class="komisi-table">
        <div class="kt-head">
          <span>Owner / Kos</span>
          <span>Booking</span>
        ...
        >
          ...
          <div class="kt-cell">
            ...
            <div style="font-size:10px;color:#8a7968;">{{ k.persen }}%</div>
          </div>
          ...
          <div class="kt-cell">
            ...
            >
              {{ konfirmasiLoading[k.id] ? '...' : 'Konfirmasi' }}
            </button>
...
export default {
  ...
  setup() {
    ...
    const fetchKomisi = async () => {
      loading.value = true
      try {
        const [komisiRes, statsRes] = await Promise.all([
          api.get('/admin/komisi'),
          api.get('/admin/komisi/ringkasan'),
        ])
        komisiList.value = komisiRes.data.data
        statistik.value  = komisiRes.data.statistik
      } catch {
    ...
    return {
      komisiList, statistik, loading, filterStatus, filteredKomisi,
...
/* STATS BESAR */
.stats-big{display:flex;gap:12px;padding:1.5rem 2.5rem;flex-wrap:wrap;background:#fff;border-bottom:1px solid #e8e2d9;}
.sb-card{flex:1;min-width:130px;border-radius:12px;padding:14px 16px;border:1px solid #e8e2d9;text-align:center;}
...
/* FILTER */
.filter-bar{padding:1rem 2.5rem;border-bottom:1px solid #e8e2d9;background:#fff;}
.filter-wrap{display:flex;gap:8px;flex-wrap:wrap;}
.filter-btn{padding:6px 14px;border-radius:20px;border:1px solid #e8e2d9;background:#fff;font-family:'DM Sans',sans-serif;font-size:12.5px;color:#6b5b47;cursor:pointer;transition:all .15s;}
...
.komisi-table{background:#fff;border-radius:14px;border:1px solid #e8e2d9;overflow:hidden;}
.kt-head{display:grid;grid-template-columns:2fr 1.2fr 1fr 1fr 1fr 1.2fr;gap:12px;padding:12px 16px;background:#f7f5f2;font-size:11px;font-weight:600;color:#8a7968;text-transform:uppercase;letter-spacing:.06em;border-bottom:1px solid #e8e2d9;}
.kt-row{display:grid;grid-template-columns:2fr 1.2fr 1fr 1fr 1fr 1.2fr;gap:12px;padding:14px 16px;border-bottom:1px solid #f5f0eb;align-items:center;transition:background .12s;}
...
@media(max-width:768px){
  .stats-big,.komisi-wrap,.filter-bar{padding-left:1.25rem;padding-right:1.25rem;}
  .kt-head,.kt-row{grid-template-columns:1fr 1fr;font-size:11px;}
  .kt-head span:nth-child(n+3),.kt-row .kt-cell:nth-child(n+3){display:none;}
}
</style>